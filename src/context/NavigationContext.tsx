...
type NavigationContextType = {
  ...
  setOriginalCameraPosition: (originalCameraPosition: {
    x: number;
    y: number;
    z: number;
  }) => void;
  setOriginalCameraTarget: (originalCameraTarget: {
    x: number;
    y: number;
    z: number;
  }) => void;
  setOriginalCameraRotation: (originalCameraRotation: {
    x: number;
    y: number;
    z: number;
    w: number;
...
export const NavigationProvider = ({ children }: NavigationProviderProps) => {
  ...
  const [originalCameraTarget, setOriginalCameraTarget] = useState({
    x: 0,
    y: 0,
    z: 0,
  });
  const [originalCameraRotation, setOriginalCameraRotation] = useState({
    x: 0,
    y: 0,
    z: 0,
    w: 1,
