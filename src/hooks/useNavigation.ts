...
const useNavigation = () => {
  ...
  const rotateCamera = useCallback(
    (x: number, y: number, z: number, w: number, duration: number) => {
  ...
  const flyBackToOriginalPosition = useCallback(
    (duration: number = 1) => {
      setIsZoomed(false);
      play();
      ...
      changeTarget(
        originalCameraTarget.x,
        originalCameraTarget.y,
        originalCameraTarget.z,
        duration,
      );
      rotateCamera(
